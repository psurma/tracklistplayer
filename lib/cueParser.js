...
function decodeCueBuffer(buf) {
  // If it starts with a UTF-8 BOM, it's definitely UTF-8
  if (buf[0] === 0xEF && buf[1] === 0xBB && buf[2] === 0xBF) {
    return buf.toString('utf8');
  }
  // Try UTF-8 first; if it contains replacement chars, fall back to Windows-1252
  const utf8 = buf.toString('utf8');
  if (!utf8.includes('\uFFFD')) return utf8;
  // Decode as Windows-1252 (superset of Latin-1 covering 0x80-0x9F)
  const win1252 = [
  ...
  for (const byte of buf) {
    if (byte >= 0x80 && byte <= 0x9F) {
      out += String.fromCharCode(win1252[byte - 0x80]);
    } else if (byte < 0x80) {
      out += String.fromCharCode(byte);
    } else {
      out += String.fromCharCode(byte); // Latin-1 range (0xA0-0xFF) maps directly
    }
  }
  return out;
}

async function parseCueFile(filePath) {
  const buf = await fsp.readFile(filePath);
  const raw = decodeCueBuffer(buf);
...
async function scanDirAsync(dir) {
  ...
  const mp3Files = files.filter((f) => AUDIO_EXTS.has(path.extname(f).toLowerCase()));
  ...
  for (const cueFile of cueFiles) {
    ...
    let cueData;
    try {
      cueData = await parseCueFile(path.join(dir, cueFile));
    } catch (_) {
...
module.exports = { parseCueFile, findCueMp3Pairs, scanDirAsync, cueTimeToSeconds };
